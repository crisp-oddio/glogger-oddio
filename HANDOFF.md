...
- **Release packaging (done)**: the extractor ships as a **PyInstaller-frozen sidecar**. `tools/model_extractor/build_sidecar.sh <triple>` freezes `extract.py` (43MB; must `--collect-all` UnityPy + fmod_toolkit + texture2ddecoder + etcpak + astc_encoder_py + archspec + pygltflib — native deps) → `src-tauri/binaries/model-extractor-<triple>[.exe]` (gitignored). Declared via `externalBin` **only in the release configs** (`tauri.release.conf.json` + the inline `--config` in `release-publish.yml`) so `tauri dev` doesn't require it; CI (`release-publish.yml`) freezes it per platform before `tauri-action`. `model_assets.rs::resolve_extractor` runs the sidecar next to the app exe in release, falling back to `python tools/model_extractor/extract.py` in dev. **Cross-platform freeze (mac/linux) unverified — first release run will confirm.**
- **Dev gotcha**: the Microsoft-Store `python.exe` is sandboxed and **cannot write `%APPDATA%\Roaming`**, so the in-app "Extract models" button fails under Store-Python in dev. The dev cache was **pre-seeded by hand** to `%APPDATA%\Roaming\glogger.Dev\models` (v2). For dev extraction use a non-Store Python.
